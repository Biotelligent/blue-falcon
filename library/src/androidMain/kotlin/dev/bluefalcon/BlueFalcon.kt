...
actual class BlueFalcon actual constructor(
    ...
    actual fun startScan(serviceUuid: String?, deviceName: String?, throttleMillis: Long, removeInactiveAfterMillis: Long): Boolean {
        ...
        // Scan build options not implemented due to issues with cheap android handsets.....
        ...
        try {
            ...
            bluetoothScanner?.let {
                ...
                it.startScan(filters, scanSettings, mBluetoothScanCallBack)
                isScanning = true
            }
        } catch (ex: SecurityException) {
            log("Scan failed $ex.message")
        } catch (ex1: Exception) {
            log("Scan failed $ex1.message")
            stopScanning()
        }
        return isScanning
    }
    ...
    private fun fetchCharacteristic(bluetoothCharacteristic: BluetoothCharacteristic, gatt: BluetoothGatt): List<BluetoothCharacteristic> = gatt.services.flatMap { service ->
    ...
    private fun setCharacteristicNotification(bluetoothPeripheral: BluetoothPeripheral, bluetoothCharacteristic: BluetoothCharacteristic, enable: Boolean, descriptorValue: ByteArray) {
        mGattClientCallback.gattForDevice(bluetoothPeripheral.bluetoothDevice)?.let { gatt ->
            fetchCharacteristic(bluetoothCharacteristic, gatt)
                .forEach {
        ...
        private fun addScanResult(result: ScanResult?) {
            result?.let { scanResult ->
                scanResult.device?.let { device ->
                    ...
                    if (scannedPeripheralList.upsert(bluetoothPeripheral)) {
                        delegates.forEach {
    ...
    inner class GattClientCallback : BluetoothGattCallback() {
        ...
        fun gattForDevice(bluetoothDevice: BluetoothDevice): BluetoothGatt? = gatts.firstOrNull { it.device == bluetoothDevice }
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt?, status: Int) {
            ...
            gatt?.device?.let { bluetoothDevice ->
                gatt.services.let { services ->
                    ...
                    bluetoothPeripheral.servicesFlow.tryEmit(services.map { BluetoothService(it) })
                    delegates.forEach {
    ...
    actual fun isBluetoothPermitted(): Boolean {
        val bluetoothPermissions = BluetoothPermissions(context)
        val isBluetoothPermitted = bluetoothPermissions.isBluetoothPermitted()
        if (!isBluetoothPermitted) {
            return false
        }

        if (bluetoothPermissions.isLocationRequired()) {
            val isLocationPermitted = bluetoothPermissions.isLocationPermitted()
    ...
    actual fun isBluetoothEnabled(): Boolean {
        return BluetoothPermissions(context).isBluetoothEnabled()
    }
}